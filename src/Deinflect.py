...
class DeinflectRule:
    def __init__(self, from_: str, to: str, type_: int, reason: DeinflectReason):
        self.from_ = from_
        self.to = to
        self.type_ = type_
        self.reason = reason
...
class DeinflectRuleGroup:
    def __init__(self, rules: List[DeinflectRule], fromLen: int):
        self.rules = rules
        self.fromLen = fromLen


deinflectRuleGroups: List[DeinflectRuleGroup] = []


def getDeinflectRuleGroups():
    if not len(deinflectRuleGroups):
        prevLen = -1
        ruleGroup = None

        for rule in deinflectRuleData:
            rule = DeinflectRule(rule[0], rule[1], rule[2], rule[3])

            if prevLen != len(rule.from_):
                prevLen = len(rule.from_)
                ruleGroup = DeinflectRuleGroup([], prevLen)

                deinflectRuleGroups.append(ruleGroup)
...
def deinflect(word: str):
    ...
    resultIndex = {}
    ruleGroups = getDeinflectRuleGroups()
    ...
    while True:
        ...
        for ruleGroup in ruleGroups:
            if ruleGroup.fromLen <= len(word):
                ending = word[-ruleGroup.fromLen::]
                for rule in ruleGroup.rules:
                    if type & rule.type_ and ending == rule.from_:
                        newWord = word[0 : len(word) - len(rule.from_)] + rule.to
                        if len(newWord) <= 1:
                            continue
                        ...
                        if resultIndex.get(newWord):
                            candidate = result[resultIndex[newWord]]
                            if candidate.type_ == rule.type_ >> 8:
                                candidate.reasons.insert(0, [rule.reason])
                                continue

                        resultIndex[newWord] = len(result)
                        # Deep clone multidimensional array
                        reasons = []
                        for array in result[i].reasons:
                            cpy = array[:]
                            reasons.append(cpy)
                        if len(reasons):
                            ...
                            if rule.reason == DeinflectReason.Causative and len(firstReason) and firstReason[0] == DeinflectReason.PotentialOrPassive:
                                firstReason = firstReason[1:]
                                firstReason.insert(0, DeinflectReason.CausativePassive)
                            else:
